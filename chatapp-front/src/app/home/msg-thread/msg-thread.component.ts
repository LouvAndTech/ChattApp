...
export class MsgThreadComponent implements OnInit {
  ...
  onScroll(){
    ...
    if (this.messageContainer.nativeElement.scrollTop == 0 && !this.loading && !this.allMessagesLoaded){
      //Get the size of the thread now
      let size = this.messageContainer.nativeElement.scrollHeight;
      //Load messages
      this.loadMore();
      //Scroll to the size of the thread before the load
      setTimeout(()=>{
        this.messageContainer.nativeElement.scrollTop = this.messageContainer.nativeElement.scrollHeight-size;
      }, 10);
    }
  }
